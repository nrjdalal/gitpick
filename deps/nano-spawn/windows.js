...
export const applyForceShell = async (file, commandArguments, options) =>
...
const shouldForceShell = async (file, { shell, cwd, env = process.env }) =>
  process.platform === "win32" && !shell && !(await isExe(file, cwd, env))
  ...
  try {
    await Promise.all(
      exeExtensions
        .flatMap((extension) =>
          [cwd, ...parts].map(
            (part) => `${path.resolve(part, file)}${extension}`,
          ),
        )
