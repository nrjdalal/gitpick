...
expectError(
  await spawn("test", { stdout: { string: "test" } as const } as const),
)
expectError(await spawn("test", { stdout: true } as const))
await spawn("test", { stderr: "pipe" } as const)
expectError(
  await spawn("test", { stderr: { string: "test" } as const } as const),
)
expectError(await spawn("test", { stderr: true } as const))
await spawn("test", { stdio: ["pipe", "pipe", "pipe"] as const } as const)
await spawn("test", {
  stdio: [{ string: "test" } as const, "pipe", "pipe"] as const,
} as const)
expectError(
  await spawn("test", {
    stdio: ["pipe", { string: "test" } as const, "pipe"] as const,
  } as const),
)
expectError(
  await spawn("test", {
    stdio: ["pipe", "pipe", { string: "test" } as const] as const,
  } as const),
)
expectError(
  await spawn("test", {
    stdio: [{ string: true } as const, "pipe", "pipe"] as const,
  } as const),
)
expectError(
  await spawn("test", {
    stdio: [{ other: "test" } as const, "pipe", "pipe"] as const,
  } as const),
)
expectError(
  await spawn("test", { stdio: [true, true, true] as const } as const),
)
...
expectError(
  await spawn("test").pipe("test", ["one"] as const, {} as const, true),
)
...
await spawn("test")
  .pipe("test")
  .pipe("test", ["one"] as const)
expectError(
  await spawn("test")
    .pipe("test")
    .pipe("test", [true] as const),
)
await spawn("test")
  .pipe("test")
  .pipe("test", {} as const)
...
await spawn("test")
  .pipe("test")
  .pipe("test", ["one"] as const, {} as const)
expectError(
  await spawn("test")
    .pipe("test")
    .pipe("test", ["one"] as const, true),
)
expectError(
  await spawn("test")
    .pipe("test")
    .pipe("test", ["one"] as const, {} as const, true),
)
