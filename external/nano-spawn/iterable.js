export const lineIterator = async function* (
  subprocess,
  { state },
  streamName,
) {
...
export const combineAsyncIterators = async function* (...iterators) {
  try {
    ...
    while (iterators.length > 0) {
      promises = iterators.map(
        (iterator, index) => promises[index] ?? getNext(iterator),
      )
      // eslint-disable-next-line no-await-in-loop
      const [{ value, done }, index] = await Promise.race(
        promises.map((promise, index) => Promise.all([promise, index])),
      )
